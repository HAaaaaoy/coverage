...
class Scenario(BaseScenario):
    def __init__(self, r_cover=0.25, r_comm=0.5, comm_r_scale=0.9, comm_force_scale=0.5):
        # agents的数量, 起飞位置, poi的数量和起飞位置
        self.num_agents = 4
        self.num_pois = 20
        self.pos_agents = [[[x, x], [x, -x], [-x, -x], [-x, x]] for x in [0.8 * r_comm]][0]
    ...
    def observation(self, agent, world):
        ...
        for poi in world.landmarks:
            pos_pois.append(poi.state.p_pos)
            pos_pois.append([poi.energy, poi.m_energy])

...
load_path = "./" + scen_name + "/" + os.listdir("./" + scen_name)[-3] + "/policy/"
...
if __name__ == '__main__':
    arglist = parse_args(scen_name, max_ep_len, num_eps, lr, gma, num_units=64)
    with U.single_threaded_session():  # 单个CPU的session
        env = make_env(scen_name, r_cover=0.25, r_comm=0.5, comm_r_scale=0.9, comm_force_scale=0.5)
        ...
        while True:
            ...
            if all(done_n) or (episode_step > arglist.max_episode_len):
                if is_save_gif:
                    if episode_step >= 50 or connect_step <= 0.9 * episode_step:
                        frames = []
                        obs_n = env.reset()
                        episode_step = 0
                        connect_step = 0
                    ...
                    anim.save(save_gif_path, writer='imagemagick', fps=10)
                    break
                else:
                    obs_n = env.reset()
                    episode_step = 0
